...
//
// For each (r_a, omega) we propagate 150 days in the Sun-Earth CR3BP and
// walk the stored accepted-step grid to find the first apogee (r > 0.4 *
// Hill), then look for the first inbound crossing of the Moon's mean
...
inline IcInfo makeIc( double r_a_km, double omega )
{
    ...
    const double T_s = 2.0 * std::numbers::pi * std::sqrt( std::pow( a_km, 3.0 ) / kGM_E );
...
TrialResult scanOne( double r_a_km, double omega )
{
    ...
    tax::ode::Solution< tax::ode::Feagin12Stepper< Vec4 >, Vec4 > sol;
    try
    {
        sol = tax::ode::propagate( Feagin12{}, rhs(), res.ic, 0.0, tFinal_days / kTimeU_days, cfg );
    ...
    for ( std::size_t i = 0; i < sol.t.size(); ++i )
    {
        ...
        } else if ( !res.reached_moon )
        {
            ...
            {
                // Linear interpolation between this step and the previous
                const double frac = ( r_prev_canon - moon_orbit ) / ( r_prev_canon - r );
                const double t_x = t_prev_canon + frac * ( t - t_prev_canon );
                // Interpolate state linearly
                const Vec4 s_x = sol.x[i - 1] + frac * ( s - sol.x[i - 1] );
...
inline Sweep runSweep( std::vector< double > r_as_km, std::vector< double > omegas_rad,
                       std::string_view label )
{
    ...
    std::cout << "[wsb_search] " << label << " grid " << sw.r_as_km.size() << " x "
              << sw.omegas_rad.size() << " = " << ( sw.r_as_km.size() * sw.omegas_rad.size() )
              << " trials..." << std::flush;

    const auto t0 = std::chrono::high_resolution_clock::now();
    for ( double r_a : sw.r_as_km )
        for ( double w : sw.omegas_rad ) sw.results.push_back( scanOne( r_a, w ) );
    ...
    std::cout << " done in " << ( sw.wall_ms / 1e3 ) << " s"
              << "  (apogee=" << n_apogee << ", Moon=" << n_moon << ", pro=" << n_pro
              << ", valid=" << n_success << " / " << sw.results.size() << ")\n";
    return sw;
}
...
inline void writeSweepArray( std::ostream& out, const std::vector< TrialResult >& v )
{
    ...
    for ( std::size_t i = 0; i < v.size(); ++i )
    {
        ...
        out << "    { "
            ...
            << "\"v_arrival_kms\": " << r.v_arrival_kms << " }" << ( i + 1 < v.size() ? "," : "" )
            << "\n";
    }
    out << "  ]";
}
...
int main()
{
    ...
    for ( int i = 0; i < N_omega_coarse; ++i )
        omegas_coarse.push_back( 2.0 * std::numbers::pi * static_cast< double >( i ) /
                                 N_omega_coarse );
    ...
    auto coarse_best_it = std::min_element(
        ...
        []( const TrialResult& a, const TrialResult& b ) { return a.score < b.score; } );
    ...
    std::vector< double > r_as_fine;
    {
        ...
        for ( double r = rmin; r <= rmax + 1.0; r += 0.0025e6 ) r_as_fine.push_back( r );
    }
    ...
    const TrialResult& best =
        ( ultra_best_it->score < fine_best_it->score )
            ? ( ultra_best_it->score < coarse_best_it->score ? *ultra_best_it : *coarse_best_it )
            : ( fine_best_it->score < coarse_best_it->score ? *fine_best_it : *coarse_best_it );
    ...
    auto sol_best = tax::ode::propagate( Feagin12{}, rhs(), best.ic, 0.0, tFinal, cfg );

    std::vector< double > t, x_syn, y_syn, x_e, y_e;
    t.reserve( sol_best.t.size() );
    x_syn.reserve( sol_best.t.size() );
    y_syn.reserve( sol_best.t.size() );
    x_e.reserve( sol_best.t.size() );
    y_e.reserve( sol_best.t.size() );
    for ( std::size_t i = 0; i < sol_best.t.size(); ++i )
    {
        const Vec4& s = sol_best.x[i];
        t.push_back( sol_best.t[i] );
        x_syn.push_back( s( 0 ) );
        y_syn.push_back( s( 1 ) );
    ...
    out << "  \"sweep_coarse\": ";
    writeSweepArray( out, sw_coarse.results );
    out << ",\n";
    out << "  \"sweep_fine\": ";
    writeSweepArray( out, sw_fine.results );
    out << ",\n";
    out << "  \"sweep_ultra\": ";
    writeSweepArray( out, sw_ultra.results );
    out << ",\n";
    ...
    out << ",\n";
    out << "    \"x_syn\":   ";
    writeArray( out, x_syn );
    out << ",\n";
    out << "    \"y_syn\":   ";
    writeArray( out, y_syn );
    out << ",\n";
    out << "    \"x_earth\": ";
    writeArray( out, x_e );
    out << ",\n";
    ...
    auto row = [&]( std::string_view label, std::string_view value ) {
        std::cout << "  " << std::string( label.size() < lw ? lw - label.size() : 0, ' ' ) << label
                  << " : " << value << '\n';
    };
