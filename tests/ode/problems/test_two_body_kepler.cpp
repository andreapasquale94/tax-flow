...
double specific_angmom( const State& s ) { return s( 0 ) * s( 3 ) - s( 1 ) * s( 2 ); }
                       ...
                       const char* label )
{
    ...
    EXPECT_NEAR( ( xfinal - x0 ).norm(), 0.0, tol_close ) << "method=" << label << " (closure)";
}
...
TEST( OdeTwoBodyKepler, Taylor16 )
{
    ...
    tax::ode::Taylor< 16, State, tax::ode::controllers::JorbaZou< double >, decltype( make_rhs() ) >
        integ{ make_rhs(), cfg };
