...
template < template < class, class, class > class IntegratorAlias >
void check_constant_term_matches_double( const char* method_name, double tol_close )
{
    tax::ode::IntegratorConfig< double > cfg;
    cfg.abstol = cfg.reltol = 1e-13;

    using IntegD = IntegratorAlias<
        StateKD, tax::ode::controllers::PI< double >,
        typename std::remove_reference_t< decltype( make_kepler_rhs< StateKD >() ) > >;
    using IntegDA = IntegratorAlias<
        StateKDA, tax::ode::controllers::PI< double >,
        typename std::remove_reference_t< decltype( make_kepler_rhs< StateKDA >() ) > >;
...
template < template < class, class, class > class IntegratorAlias >
void check_linear_term_matches_analytical_stm( const char* method_name, double tol_abs )
{
    tax::ode::IntegratorConfig< double > cfg;
    cfg.abstol = cfg.reltol = 1e-13;

    using IntegDA = IntegratorAlias<
        StateHDA, tax::ode::controllers::PI< double >,
        typename std::remove_reference_t< decltype( make_harmonic_rhs< StateHDA >() ) > >;
    ...
    for ( int row = 0; row < 2; ++row )
    {
        for ( int col = 0; col < 2; ++col )
        {
            ...
            EXPECT_NEAR( lhs, rhs, tol_abs )
                << "method=" << method_name << " row=" << row << " col=" << col;
        }
    }
}
