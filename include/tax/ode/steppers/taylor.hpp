...
//
// The per-step Taylor expansion is stored in StepData and evaluated
// via tax::la::eval(e, tau).
...
struct TaylorStepper
{
    ...
    // StepData: per-step Taylor expansion of x(t) in time around the
    // step start. We store one tax::TE<N> per state component.
    // Evaluate via tax::la::eval(expansion, tau).
    using TE = tax::TE< N, 1 >;
    using StepData = tax::la::VecNT< D, TE >;

    template < class F >
    [[nodiscard]] StepResult< StateT, TaylorStepper > step( const F& f, const StateT& x, T t, T h,
                                                            const Config& cfg );
...
// -------- step (real Taylor expansion) --------
template < int N, class StateT, class Controller >
template < class F >
StepResult< StateT, TaylorStepper< N, StateT, Controller > >
TaylorStepper< N, StateT, Controller >::step( const F& f, const StateT& x, T t, T h,
                                              const Config& cfg )
{
    using std::abs;
    using std::pow;
    ...
    StateTE x_te{ dim };
    for ( Eigen::Index i = 0; i < dim; ++i ) x_te( i ) = TE::constant( x( i ) );
    ...
    const T err_norm =
        c_N_norm * pow( abs( h ), T( N ) ) + c_Nm1_norm * pow( abs( h ), T( N - 1 ) );

    T x_norm = T{ 0 };
    for ( Eigen::Index i = 0; i < dim; ++i ) x_norm = std::max( x_norm, T( abs( x_new( i ) ) ) );
    const T tol = cfg.abstol + cfg.reltol * x_norm;

    // --- 6. Step-size control via the Taylor dispatch helper.
    const auto [h_next, accepted] =
        detail::select_taylor_step< N >( controller_, h, c_N_norm, c_Nm1_norm, err_norm, tol );
